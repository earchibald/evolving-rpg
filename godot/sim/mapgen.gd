...
## tests/core/path-pull.test.ts (5): 1 ported, 4 deferred to Wave E's
##   `sim/commands.gd` (Task 2.E3) — each drives `createWorld`, which places
##   the relic on the walked path; `walkPath` is the ruler those cases hold up
##   to it, not their subject. Named individually in test_path_pull.gd.
