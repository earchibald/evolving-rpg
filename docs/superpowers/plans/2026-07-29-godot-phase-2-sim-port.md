...
- [ ] **Step 3: RED. Step 4: Port the reducer, one event case at a time,** running the suite after each case so a divergence is attributable to the case that introduced it. Hazards, each of which has already cost this repo a debugging session:
  ...
  - **Never store a `SimTables` float return raw into state.** `SimTables.bounty_stretch` returns a **float** (1.5, 2.5) and `mean_damage` returns a **mean**. Those are correct — they are coefficients. But `SimCanonical.encode` refuses a fractional number, and the state's numbers are all integers, so a coefficient that reaches `GameState` un-rounded either crashes the encoder or forks the hash. Store only the integer *result* of a calculation involving them, and round with the same operation the reference uses at that site (this file uses `floor`, `round` and `ceil` in different places — check each). Task 2.B3's review flagged this as an assumption it could not verify because this task did not exist yet; verifying it is now yours. A cheap guard: after `apply`, walk the returned state and assert no number in it is fractional.
